...
int main()
{
    ...
    printf("\n\nPUNTERO A ESTRUCTURA NOTACION PUNTERO:");//LO MISMO QUE LA FLECHA PERO MAS ENGORROSO
    ...
    for(i=0; i<2; i++){
        printf("DNI: ");
        scanf("%ld", &(*(ptrPers+i)).dni); //&(ptrPers+i)->dni

        printf("\nNOMBRE: ");
        setbuf(stdin, NULL);
        scanf("%[^\n]", (*(ptrPers+i)).nombre);   //(ptrPers+i)->nombre no le paso & por que es un string    %[^\n]==EXPRESION REGULAR
